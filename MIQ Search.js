...
(function() {
  ...
  const wantDates = DATES.map(d => new Date(d));
...
`;
  ...
  function getAvailableDates(dates) {
    ...
    for (let a = 0; a < n.length; a++) {
      ...
      var i = n[a].substring(0, 4) + '-' + n[a].substring(4, 6) + '-' +
          n[a].substring(6, 8);
  ...
  function lookForDates() {
    ...
    if (dates) {
      ...
      const match = ANY_DATE ? dateArray : DATES.filter(
          value => dateArray.includes(value));
